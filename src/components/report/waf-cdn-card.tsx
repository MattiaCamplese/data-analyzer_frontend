...
function Service({ label, count, assets, icon: Icon }: {
    ...
    <div className="flex flex-col gap-1.5">
      <div className="flex items-center justify-between text-sm">
        ...
        {active ? (
          ...
          <span className="rounded-full bg-muted px-2 py-0.5 text-xs text-muted-foreground">Non rilevato</span>
        )}
      </div>
...
export function WafCdnCard({ waf, cdn }: WafCdnCardProps) {
  return (
    <Card className="shadow-none dark:ring-0">
      <CardHeader>
        <CardTitle className="text-sm">WAF & CDN</CardTitle>
        <CardDescription>Protezione e distribuzione</CardDescription>
      </CardHeader>
      <CardContent className="flex flex-col gap-4">
        <Service label="WAF" count={waf.count} assets={waf.assets} icon={waf.count > 0 ? ShieldCheck : ShieldOff} />
        <div className="border-t" />
        <Service label="CDN" count={cdn.count} assets={cdn.assets} icon={Wifi} />
      </CardContent>
    </Card>
  );
}
